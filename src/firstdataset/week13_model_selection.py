...
from .week12_uncertainty_analysis import SELECTIVE_COVERAGE_LEVELS, run_week12_uncertainty_analysis
...
def build_week13_scoreboard(
    *,
    ...
    predictions, metrics, selective, _ = run_week12_uncertainty_analysis(
...
def build_week13_report_markdown(scoreboard: pd.DataFrame) -> str:
    ...
    lines = [
        "# Week 13 Final Model Selection Report",
        "",
        "## Objective",
        "",
        "Week 13 focuses on final model selection through reliability analysis rather than raw performance alone. The goal is to compare model and feature-set combinations on accuracy, calibration, uncertainty behavior, selective prediction, and cross-environment robustness in order to recommend a final model that is not only accurate, but also trustworthy.",
        "",
        "## Approach",
        "",
        "The Week 13 pipeline reuses the Week 11 feature sets and the Week 12 uncertainty analysis outputs. Each candidate combines one model and one feature set. Candidates are evaluated under both stratified cross-validation and cross-environment validation. The comparison includes accuracy, ROC-AUC, Brier score, log loss, expected calibration error, uncertainty separation between correct and incorrect predictions, selective prediction performance at reduced coverage, and cross-environment failure behavior.",
        "",
        "## Headline Findings",
        "",
...
def build_week13_summary_text(scoreboard: pd.DataFrame) -> str:
    top_three = scoreboard.head(3)
    lines = [
        "Week 13 Final Model Selection Summary",
        "",
        "Top 3 candidates:",
    ]
...
def write_week13_charts(
    ...
    ax.set_title("Week 13 Overall Reliability Score by Candidate")
    ax.set_xlabel("Composite reliability score")
    fig.tight_layout()
    path_1 = output_path / "week13_overall_scoreboard.png"
    ...
    fig.tight_layout()
    path_2 = output_path / "week13_accuracy_calibration_tradeoff.png"
    ...
    ax.set_title("Week 13 Normalized Reliability Components")
    ...
    fig.tight_layout()
    path_3 = output_path / "week13_metric_heatmap.png"
    ...
    ax.set_title("Week 13 Selective Prediction for Top Candidates")
    ax.set_xlabel("Coverage kept")
    ax.set_ylabel("Accuracy")
    ax.set_ylim(0.0, 1.0)
    ax.legend(loc="lower left", fontsize=8)
    fig.tight_layout()
    path_4 = output_path / "week13_selective_top_candidates.png"
