...
from .descriptor_graph_model import run_descriptor_graph_prototype_on_split


@dataclass(frozen=True)
class CrossEnvironmentFoldResult:
    model_name: str
    environment_id: int
...
def run_cross_environment_validation_for_arrays(
    ...
    *,
    ...
    random_state: int = 42,
) -> tuple[list[CrossEnvironmentFoldResult], pd.DataFrame]:
    ...
    results: list[CrossEnvironmentFoldResult] = []
...
def write_cross_environment_charts(summary: pd.DataFrame, output_dir: str | Path) -> list[Path]:
    ...
    ax.set_title("Cross-Environment Mean Performance")
    ax.set_ylabel("Score")
    ax.set_xlabel("Model")
    ax.legend(loc="lower right")
    fig.tight_layout()
    path_1 = output_path / "mean_scores.png"
    ...
    ax.set_title("RB Recall by Held-Out Environment")
    ax.set_xlabel("Model")
    ...
    fig.tight_layout()
    path_2 = output_path / "rb_recall_heatmap.png"
