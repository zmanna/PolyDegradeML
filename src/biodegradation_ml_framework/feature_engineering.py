...
def build_feature_engineering_bundle() -> FeatureEngineeringBundle:
    ...
    return FeatureEngineeringBundle(
...
def run_feature_engineering_evaluation(*, random_state: int = 42) -> tuple[pd.DataFrame, pd.DataFrame]:
    bundle = build_feature_engineering_bundle()
    diagnostics_base, results_base = run_cross_validation_for_arrays(
        bundle.baseline_X.to_numpy(dtype=np.float64),
        bundle.y_binary,
        random_state=random_state,
    )
    diagnostics_enhanced, results_enhanced = run_cross_validation_for_arrays(
...
def write_feature_engineering_chart(results: pd.DataFrame, output_path: str | Path) -> Path:
    summary = (
    ...
    axes[0].set_title("Feature Engineering Accuracy Comparison")
    ...
    axes[1].set_title("Feature Engineering RB Recall Comparison")
