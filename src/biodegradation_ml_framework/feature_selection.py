...
from .environment_validation import run_cross_environment_validation_for_arrays
from .cross_validation import run_cross_validation_for_arrays
from .feature_engineering import TIER1_BASELINE_FEATURES, TIER2_PROXY_FEATURES, build_feature_engineering_bundle


@dataclass(frozen=True)
class SelectedFeatureSets:
...
def build_selected_feature_sets(ranking: pd.DataFrame) -> SelectedFeatureSets:
...
def evaluate_feature_sets(*, random_state: int = 42) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, SelectedFeatureSets]:
    bundle = build_feature_engineering_bundle()
    ranking = compute_feature_rankings(random_state=random_state)
    feature_sets = build_selected_feature_sets(ranking)
    ...
    for feature_set_name, frame in feature_map.items():
        diagnostics, results = run_cross_validation_for_arrays(
...
def write_feature_selection_charts(
    ranking: pd.DataFrame,
    ...
    fig.tight_layout()
    path_1 = output_path / "top_feature_importance.png"
    ...
    axes[0].set_title("Feature Set Accuracy with SMOTE")
    axes[0].set_ylabel("Accuracy")
    axes[0].set_xticks(x)
    axes[0].set_xticklabels(labels, rotation=30, ha="right")

    axes[1].bar(x, focused["rb_recall"], width=0.6, color="#E15759")
    axes[1].set_title("Feature Set RB Recall with SMOTE")
    axes[1].set_ylabel("RB Recall")
    axes[1].set_xticks(x)
    axes[1].set_xticklabels(labels, rotation=30, ha="right")

    fig.tight_layout()
    path_2 = output_path / "feature_set_comparison.png"
